...
All 123 public shared operations attach an
...
and all 123 are `stable`; the bounce family, the campaign preview, test-send,
...
opt-in resend, and the system identity and diagnostics reads joined the
...
The runtime-operation bridge infrastructure is now empty — all 123
operations have standalone product-domain contracts. The final root
build loads all 123 runtime operations and validates them against their
