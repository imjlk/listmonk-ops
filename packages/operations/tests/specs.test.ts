...
describe("email operations specification", () => {
	...
	test("preserves the standalone catalog order across module splits", () => {
		expect(standaloneOperationSpecs.map(({ id }) => id)).toEqual([
			"templates.reconcile",
			"templates.create",
			"templates.update",
			"templates.delete",
	...
	test("models every public shared operation with governed contracts", () => {
		...
		expect(operationIds).toHaveLength(104);
		expect(new Set(operationIds).size).toBe(104);
		expect(
