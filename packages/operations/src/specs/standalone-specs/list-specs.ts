		...
		cases: [
			{
				...
				semantics: {
					kind: "safe",
					reason:
						"The key is bound to the created list id in a durable store before the response returns; an identical retry (same key, same request payload, same Listmonk target) replays that list with created: false, and a different request or target under the same key is rejected.",
				},
			},
			{
