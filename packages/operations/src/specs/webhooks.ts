		...
		cases: [
			{
				...
				semantics: {
					...
					idempotent: false,
					reason:
						"The probe derives its event id as an HMAC over a server-generated high-entropy probe id key persisted with the webhook store — key separation from every endpoint signing credential: probe identity is decoupled from the signing secret, and a low-entropy signing secret cannot be brute-forced from known probe message/id pairs — and bound to the endpoint's configuration revision, so the outbox dedup collapses an identical retry onto the already-queued delivery and replays or resumes it; the persisted event durably records the correlation id. The POST itself stays at least once: a retry or expired lease whose first attempt already reached the endpoint redelivers the ping, and a pruned original lets the repeat send a fresh probe, so the outcome is verified through webhooks.delivery.list with the stable event-id header available for receiver deduplication. A configuration change re-keys the id by design, so the repeat tests the new configuration.",
				},
			},
			{
