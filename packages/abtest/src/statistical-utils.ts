 ...
 * Provides sample size calculations and validation
 */
export class StatisticalUtils {
	...
	static calculateMinimumSampleSize(
		baselineConversionRate: number = 0.05, // 5% baseline conversion rate
		minimumDetectableEffect: number = 0.2, // 20% relative improvement
		alpha: number = 0.05, // 5% significance level
		beta: number = 0.2, // 20% type II error (80% power)
	): number {
	...
	/**
	 * Get sample size recommendation based on total subscribers
	 */
	static getSampleSizeRecommendation(
		...
		const minimumSamplePerVariant = this.calculateMinimumSampleSize(
			baselineConversionRate,
			minimumDetectableEffect,
		);

		// Calculate current test group configuration
		...
		// Calculate recommended test percentage
		...
		// Calculate absolute minimum test percentage (for statistical significance)
		const minimumTestPercentage = Math.max(
			1,
			Math.ceil((totalMinimumSampleNeeded / totalSubscribers) * 100),
		);

		// Calculate statistical power with current configuration
	...
	private static calculateStatisticalPower(
		...
		// Convert to power
		const power = this.standardNormalCDF(zBeta);
	 ...
	 */
	static validateTestConfiguration(
		totalSubscribers: number,
		...
		const sampleSizeRecommendation = this.getSampleSizeRecommendation(
			totalSubscribers,
			testPercentage,
			variantCount,
		);

		// Add warnings from sample size analysis
		if (!ignoreWarnings) {
