...
Transactional hardening: tighten recipient validation to exactly one of subscriber_email or subscriber_id (XOR), reject header values that smuggle CR/LF/NUL or other control characters, block reserved transport headers, and add an optional client-side `idempotency_key` field that Listmonk does not enforce.
