...
116개 공용 shared Operation 모두 `spec` descriptor를 포함합니다. Spec은
...
`subscribers.import.stop`)은 관찰된 Listmonk 6.2 응답 형태를 로컬 스택으로
...
Operation 116개 전체, API 경계 규칙, 0개 governed runtime bridge,
stable compatibility baseline(116개)과 spec-to-runtime 직접 graph edge
353개를 검증합니다.

116개 shared Operation은 모두 독립적인 TypeScript 계약을 사용합니다. 다시
