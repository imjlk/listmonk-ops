...
명시적 `before` cutoff를 받아 retry가 확인된 삭제 창을 정확히 재사용하도록
`webhooks.prune`의 retry를 결정론적으로 만들고 함께 승격했습니다.
...
history를 명시적으로 확인한 경우에만 삭제합니다. `--before`에 이전 실행이
보고한 timestamp를 전달하면, 확인된 retry가 현재 시계로 cutoff를 다시 계산하지
않고 동일한 창을 정확히 삭제합니다.
