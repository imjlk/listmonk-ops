...
삭제 도구 `listmonk_delete_bounce`는 기존 이름을 유지하면서 동일한 공용
operation을 structured content로 노출합니다. `bounces.delete`는 명시적인
확인이 필요하며, Listmonk는 이미 삭제된 ID도 동일하게 성공으로 확인하기
때문에 재시도는 `bounces.list`로 검증하는 문서화된 no-op입니다.

`bounces.prune`은 확인 게이트가 없던 기존 대량
`listmonk_delete_bounces` 도구(명시적 ids 또는 `all=true`)를
미리보기-echo 계약으로 대체합니다. dry run은 제한된 선택 창(최대
100개)의 정확한 바운스 id를 보고하고, 파괴적 실행은 그 집합을
`bounce_ids`로 echo합니다. Listmonk는 없는 ID가 하나라도 포함된 대량
삭제를 거부하므로 파괴적 실행은 per-id 삭제로 발행되며, 없는 ID도
동일하게 성공으로 확인하기 때문에 echo된 반복은 아무것도 새로 삭제하지
않습니다. hygiene 미리보기와 마찬가지로 prune은 dry run에도 명시적인
확인이 필요합니다.
...
checkpointing하고 재개 시 결정적인 `abtest:` 태그로 campaign을 조정하며,
assignment seed와 auto-launch 창을 원격 효과 전에 기록하고, 저장된 seed 아래
크래시된 시도의 태그된 대상 리스트를 채택하며, 확인된 삭제만 롤백해 모든
크래시 창을 닫고 승격했습니다. 열한 번째 batch에서는 `subscribers.create`를
...
envelope, id 없는 응답의 uuid 상관). 스물한 번째 batch에서는
`templates.create`를 같은 방식으로 승격했습니다(템플릿 레코드에는 uuid가
없어 생성 응답의 id가 바인딩에 필요). 스물두 번째 batch에서는
`media.upload`(CLI `--idempotency-key`)를 승격했습니다 — payload hash는
...
스물여섯 번째 batch에서는 `abtest.tick`을 같은 상태 바인딩 복구 계약으로
승격했습니다(echo가 원래 due였던 각 테스트의 틱 전 상태를 담고 재시도는
정확히 그 집합만 재시도하며, 실패한 틱은 클레임 집합을 구조화 에러
세부사항으로 노출합니다 — winner 배포는 외부에 보이므로 해당 복구
경우는 여전히 reconcile로 분류합니다).
스물네·스물다섯 번째 batch에서는 `sequences.tick`과 `webhooks.tick`을
같은 echoed-claim-set 복구 계약으로 승격했습니다 — 출력이 정확한 클레임
레코드와 원래 클레임된 위치(시퀀스는 단계, 웹훅은 시도 횟수)를 echo하고,
그 집합을 `recovery_set`(CLI `--recovery-set`)으로 실은 재시도는 새
...
`record-conversion`은 발송 시작 이후, 테스트 종료 시각에 사전 등록된 귀속 기간을
더한 시각까지의 이벤트만 기록합니다. 귀속 기간이 없으면 테스트 종료 시각,
그것도 없으면 발송 시작 후 72시간을
적용합니다. 기록 시점에도 이 기한이 지나지 않아야 합니다. 구독자가 해당 변형의 프로비저닝 목록에 있는지도 확인합니다.
로컬 저장소 전체에서 이벤트 ID는 고유하며, 동일한 재시도는 `duplicate`를
반환하고 같은 ID의 다른 내용은 거부합니다. CLI와 MCP는 트랜잭션으로 기록하는
...
- `--enable-stratification` — 구독자를 이메일 도메인 제공자별로 분류하고
  제약된 할당량 행렬을 실제 수신자 슬라이스에 적용하여 각 제공자
  층(stratum)이 모든 변형/홀드아웃 그룹의 비례 배분을 받도록 합니다. 각
  층 안에서 구성원은 동일한 결정론적 digest 순서로 정렬되고 각 그룹은 해당
  할당량 칸을 채우므로, 테스트/홀드아웃과 변형별 총량은 여전히
  largest-remainder 매니페스트에서 오고 할당은 저장된 시드 아래 재현
  가능합니다. 이메일이 없는 구독자가 하나라도 있거나 할당량 계산이 실패하면
  프로비저닝은 층화 정의 없이 매니페스트 할당으로 되돌아갑니다.
...
사전 검사는 본문의 단어나 중괄호 개수 대신, 공통 템플릿 푸터를 포함한
서버 렌더링 미리보기를 검사합니다. HTML 앵커 또는 일반 텍스트 URL에
Listmonk 기본 구독 관리 경로가 있어야 통과합니다. 미리보기 오류나
비어 있거나 과도하게 큰 결과는 검사 실패로 처리합니다.

링크 상태 검사는 렌더링된 일반 링크와 일반 텍스트 미리보기의 URL을 대상으로
하며 구독 관리·옵트인·
추적 경로, 인식 가능한 인증 경로와 자격 증명 쿼리 링크는 방문하지 않습니다.
동작을 알 수 없는 사용자 정의 액션 링크가 있으면 `check_links`를 끄세요.

이 검사는 샘플 렌더링 기준이며 모든 구독자별 조건부 분기, CSS 가시성,
사용자 정의 해지 서비스까지 보장하지 않습니다. 기본 경로의 존재를 검사할
뿐 외부 해지 엔드포인트의 소유권·접속 가능성을 증명하지 않으며, 영구적인
발송 승인 토큰을 만들거나 이후 발송을 원자적으로 처리하지 않습니다.

### 시퀀스의 리스트별 수신 조건

뉴스레터 발송 단계에 `consent_list_ids`를 추가합니다. CLI의
`sequences create/update` JSON과 MCP에서 같은 단계 스키마를 사용합니다.

```json
{"id":"newsletter","type":"send","template_id":1,"consent_list_ids":[42]}
```

지정한 모든 리스트에서 현재 수신이 허용되어야 합니다. 구독 해지 또는
멤버십 부재 시 발송 전에 등록 건을 취소하며, 다른 리스트의 구독으로
대체하지 않습니다. confirmed 상태는 허용하고, unconfirmed 상태는 최신
리스트 조회 결과가 단일 옵트인일 때만 허용합니다. 설정이나 리스트 조회
권한을 확인할 수 없으면 발송하지 않습니다. 전역 차단은 항상 우선합니다.
등록 이후에도 매 발송 단계 직전에 다시 검사합니다.

**전환 시 주의:** 필드가 없는 기존 단계는 트랜잭셔널 동작을 유지하므로
뉴스레터 목적에는 조건을 명시적으로 추가해야 합니다. 기존 등록 건은
...
가드의 정지는 캠페인 버전이 아니라 관찰한 `running` 상태에 묶입니다.
Listmonk는 발송 중인 캠페인의 `updated_at`을 발송 배치마다 갱신하므로, 버전
일치를 요구하면 반송이 빠르게 쌓이는 발송에서 정지가 자주 실패하기
때문입니다. 공통 상태 전이 규칙이 쓰기 직전에 캠페인을 다시 읽어, 여전히
running이면 정지하고 이미 paused이면 아무것도 하지 않으며, 그사이 finished나
cancelled가 됐으면 상태 변경 요청 없이 실패합니다. scheduled 캠페인은 직접
정지하지 않습니다. 수동 `campaigns pause --expected-updated-at`은 정확한 버전
검사를 그대로 유지하므로, 발송 중인 캠페인은 이 옵션 없이 정지하세요.
