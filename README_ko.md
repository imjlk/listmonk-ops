...
124개 공용 shared Operation 모두 `spec` descriptor를 포함합니다. Spec은
...
(`subscribers.send-optin`), 자격 증명 무권화 설정 읽기(`settings.get`)은
...
Operation 124개 전체, API 경계 규칙, 0개 governed runtime bridge,
stable compatibility baseline(124개)과 spec-to-runtime 직접 graph edge
377개를 검증합니다.

124개 shared Operation은 모두 독립적인 TypeScript 계약을 사용합니다. 다시
