...
열일곱 번째 batch에서는 `ops.subscribers.hygiene`를 승격했습니다. 파괴적
실행은 dry-run이 보고한 정확한 후보 구독자 집합을 echo하고(CLI
`--subscriber-ids`), 재시도에서 자격을 벗어난 구독자는 건너뛰며 winback
리스트 추가는 구독자별 idempotent 멤버십이라 동일한 재시도가 새 효과를
만들지 않습니다.
