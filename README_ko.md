...
않고, 애매하게 끝난 시도는 자신의 claim을 unknown으로 표시해 다음 재시도가
즉시 회복해 리스트 이름·속성·생성 시각으로 조정합니다. settled miss가 크래시한
생성이 관측되지 않음을 확인한 뒤에만 신규 생성하고, 동명 매치가 해당 시도의
산물일 가능성이 있으면 키를 수동 조정 대상으로 남깁니다(이름은 소유 증명이
아닙니다). 키 기반 생성에는 이 저장소가 필수이므로 저장소가 없는 surface는 보장을
