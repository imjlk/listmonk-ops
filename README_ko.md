...
번째 batch에서는 `webhooks.delivery.retry`(이미 pending인 delivery에 대한 반복은
`retried: false`로 추가 변경 없이 재생)와 조건부 retry 시맨틱의
`ops.segments.drift`(키 있는 샘플은 기간을 교체해 수렴, 키 없는 append는
unsafe)를 승격했습니다.
